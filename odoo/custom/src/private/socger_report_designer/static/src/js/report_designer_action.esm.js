...
export class ReportDesignerAction extends Component {
    ...
    async loadReactApp() {
        try {
            ...
            // Read the version manifest to find the current hashed bundle filename.
            // This bypasses browser HTTP cache when the bundle is rebuilt.
            const entry = await this.fetchEntryFromManifest();

            // Load the React bundle dynamically
            await this.loadScript(`/socger_report_designer/static/dist/${entry}`);
    ...
    /**
     * Fetch the version manifest generated by Vite to find the hashed
     * entry filename. Falls back to the unhashed name if the manifest
     * is missing (e.g. older builds).
     */
    async fetchEntryFromManifest() {
        try {
            const res = await fetch(
                "/socger_report_designer/static/dist/version.json",
                {cache: "no-store"}
            );
            if (res.ok) {

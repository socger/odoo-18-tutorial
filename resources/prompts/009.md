...
- Crear modelo: fleet.vehicle.document.category ... modelo a crear en módulo
  socger_expand_fleet
  - Añadir fields:
    - field: name, type: string, required: True, tracking: True
    - field: fleet_vehicle_document_category_id, type: Many2one con tabla
      fleet_vehicle_document_category, required: False, tracking: True. Este field
      servirá para saber si es una categoría hija de otra categoría. Necesitamos que
      este field en las vistas se presente con el valor del name de la categoría padre.
  - Añadir índices:
    - Por el field "name", índice único: Si
  ...
  - Crear un submenú llamado "Flota - Documentación" bajo el menú "Configuración". Este
    submenú tendrá un estilo parecido al submenú "Modelos" que hay en este mismo menú
    "Configuración". De este submenú que vas a crear crearás otro submenú con el título
    "Categorías" y cuando lo pulses llamará a la vista de tipo list del modelo
    "fleet.vehicle.document.category".

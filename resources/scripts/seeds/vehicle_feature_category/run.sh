#!/usr/bin/env bash
# Seed vehicle.feature.category records into the Odoo `devel` database
...
# `socger_expand_fleet` module installed (table vehicle_feature_category
# exists).
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SEED_FILE="$SCRIPT_DIR/seed_vehicle_feature_category.sql"
...
# resources/scripts/seeds/vehicle_feature_category/).
...
echo "Loading vehicle_feature_category seed data into 'devel' (container db)..."
...
echo "    -c 'SELECT id, name, description FROM vehicle_feature_category ORDER BY id;'"
